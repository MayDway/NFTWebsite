		...
		if(@$_REQUEST['action']=='edit' && @$_REQUEST['editid']!="")
	    {
	        ...
	        $selectproduct=mysqli_query($conn,"SELECT * FROM products WHERE product_id='$edid'");
	        $rowproduct=mysqli_fetch_assoc($selectproduct);
	        
	    }
		

	if($_REQUEST['page']=='productedit' && isset($_POST['btnedit']) && $_REQUEST['action']=='edit')
	{
		$edid=$_REQUEST['editid'];
		...
		if($image!="")
		{
			...
			$inspd=mysqli_query($conn,"UPDATE products SET title='$title',name='$productname',image='$image',description='$description',price='$price' WHERE product_id='$edid'");
			
		}
		else
		{
			$inspd=mysqli_query($conn,"UPDATE products SET title='$title',name='$productname',image='$oldimage',description='$description',price='$price' WHERE product_id='$edid'");
		}
					...
					<div class="pd-20 card-box mb-30">
						<div class="clearfix">
							<div class="pull-left">
								...
								<?php if($rowproduct['image']!=""){ ?>
								<img src="src/images/products/<?php echo $rowproduct['image']; ?>" id="" class="productimage rounded-circle" alt="Preview">
								<?php }else{ ?>
						...
						<form method="post" id="productForm" enctype="multipart/form-data">
							<div class="form-group row">
								...
								<div class="col-sm-12 col-md-10">
									<input
										...
										type="text"
										placeholder=""
										name="title" value="<?php echo $rowproduct['title']; ?>"
									/>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-sm-12 col-md-10">
									<input
										...
										type="text"
										placeholder=""
										name="productname" value="<?php echo $rowproduct['name']; ?>"
									/>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-sm-12 col-md-10">
									...
									<input type="hidden" name="oldimage" value="<?php echo $rowproduct['image']; ?>">
								</div>
							</div>
							<div class="form-group row">
								<label class="col-sm-12 col-md-2 col-form-label">Description <span style="color: red;"> *</span></label>
								<div class="col-sm-12 col-md-10">
									<textarea name="desc" class="form-control"><?php echo $rowproduct['description']; ?></textarea>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-sm-12 col-md-10">
									<input
										class="form-control"
										type="number" name="price" value="<?php echo $rowproduct['price']; ?>"
									/>
								</div>
							</div>
							...
							<div class="form-group row ml-auto">
								...
								> -->
								<button type="submit" class="btn btn-primary btn-sm" name="btnedit">Submit</button>
							</div>
						</form>
